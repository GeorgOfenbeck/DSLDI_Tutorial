        ...
        val f: CVector[V,E,R,T] => CVector[V,E,R,T] = (IR.id2tp(a).rhs, IR.id2tp(b).rhs) match {
          case (IR.ConstDef(I(n)), _) => {
            (in: CVector[V,E,R,T]) => {
              ...
              val A = fmap(b)
              in.GT(A,h,h,Vector(n,size/n))(in) //in.grouped(size / n).flatMap(chunk => fmap(b)(chunk)).toVector
            }
          }
          case (_, IR.ConstDef(I(n))) => {
            (in: CVector[V,E,R,T]) => {
              ...
              val h: (Vector[Int]) => Int = (loopvars: Vector[Int]) => { //this is the stride function we access with
                  base + loopvars.zip(strides).foldLeft(0)({(acc,ele) => acc + (ele._1 * ele._2)})}
              val A = fmap(b)
              in.GT(A,h,h,Vector(n,size/n))(in) //in.grouped(n).toList.transpose.map(chunk => fmap(a)(chunk.toVector)).transpose.flatten.toVector
            }
          }
